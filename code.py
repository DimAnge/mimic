...
i2c = busio.I2C(board.GP1, board.GP0, frequency=400_000)
...
def render_system():
    if not status_ok:
        ...
        sys_foot_label.text = "No wifi" if not online() else "Bridge offline"
        draw_bar(cpu_bar, 0)
        draw_bar(ram_bar, 0)
        return
...
while True:
    ...
    if (mode not in ("USAGE", "SYSTEM", "SPOTIFY", "CALENDAR", "GAMES")
            and now - last_status_fetch > BACKGROUND_REFRESH):
        fetch_status(now)
